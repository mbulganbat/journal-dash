...
- **Auth**: Clerk. `<ClerkProvider>` wraps the root in `index.tsx`; `App.tsx` gates the whole app shell behind `<SignedIn>`/`<SignedOut>`. **Demo mode** (2026-07-02): the sign-in gate offers "View live demo" — renders the same `AppShell` under `context/DemoProvider.tsx` (mock data from `data/mockTrades.ts`, every mutator toasts "Demo mode" and persists nothing, sessionStorage `lumex_demo` keeps it across refresh, floating `DemoBanner` exits back to the gate; a real sign-in always clears the flag). The shared shell lives in `components/layout/AppShell.tsx`. Publishable key in `frontend/.env` (gitignored) as `VITE_CLERK_PUBLISHABLE_KEY`. **A `frontend/.env.example` now exists** — copy it to `.env` and fill in a real key on any fresh clone.
- **State**: single React Context (`frontend/context/AppContext.tsx`, now 88 lines) backed by **Supabase Postgres** (project `vzebhiqvanuxtzfgfeow`, "Trading journal", ap-northeast-2). Clerk is the auth source via Supabase **Third-Party Auth** — every request carries the Clerk session token (`lib/supabase.ts`), RLS keys rows to the JWT `sub` claim. Data flow: `lib/db/*` repos (camelCase↔snake_case mappers + generic `createRepo`) → `context/hooks/useSyncedCollection.ts` (optimistic updates, toast on failure) → `context/hooks/useAppData.ts` (orchestration) → provider. localStorage now only holds `lumex_selected_account` (device-local UI pref); stale legacy `lumex_*` keys may linger in old browsers but nothing reads them. New users start empty — no mock seeding, and the one-time localStorage→DB import was **removed the same day it ran** (it had already lifted the owner's data, and leaving it would have poured old local mock data into any *new* account created on a browser with leftover keys).
...
- `Sidebar.tsx`: bottom row has the real profile info (avatar/name/plan), clicking it navigates to Settings. (The separate Clerk `<UserButton>` that used to sit next to it was later removed — account management now lives in Settings → Account's "Sign-in & Security" block: Clerk user info + "Manage account" (`openUserProfile()`) + "Sign out" (`signOut()`), all app-styled. Clerk theming is global: `lib/clerkAppearance.ts` passed to `<ClerkProvider appearance>`, so the sign-in card and the account modal are dark automatically.)
...
### 7.2 Vercel deployment — ✅ FULLY DONE (2026-07-02)
**Live at https://lumex-trading-journal.vercel.app** (Vercel project `lumex-trading-journal`, personal scope `mbulganbats-projects`, deployed via `vercel deploy --prod` CLI — the canonical prod alias is public; hash/branch URLs sit behind Vercel SSO, which is normal Standard Protection). Config: root `vercel.json` (workspace build → `frontend/dist`, SPA rewrite, `experimentalServices.frontend` added by CLI v54 — the unused `backend` service it also auto-added was removed on purpose) + committed `frontend/.env.production` (publishable-only values; they ship in the JS bundle regardless, verified baked in). Clerk still runs the **dev instance** on the live URL ("Development mode" badge) — moving to a Clerk production instance requires a custom domain (vercel.app can't host one), which is a future step, not a blocker.
